...
make_tag_folders = (
    True  # Exports to folders using first tag only, if `multi_tag_folders = False`
)
...
export_image_repository = True  # Export all notes as md but link images

HOME = os.getenv("HOME", "")
default_out_folder = os.path.join(HOME, "Desktop", "BearOutput")
default_backup_folder = os.path.join(HOME, "Desktop", "BearBackup")
...
export_path = default_out_folder
no_export_tags = ["#blog"]
...
def sub_path_from_tag(temp_path, filename, md_text):
    ...
    if multi_tag_folders:
        ...
        for matches in re.findall(pattern1, md_text):
            tag = matches[0]
            print(tag)
            if tag.lower().startswith("10"):
                tags.append(tag)
        for matches2 in re.findall(pattern2, md_text):
            tag2 = matches2[0]
            print(tag2)
            if tag2.lower().startswith("10"):
                tags.append(tag2)
        if len(tags) == 0:
